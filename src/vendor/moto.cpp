...
GLvoid moto::update() {
	...
	if (state->s_down && state->speed > -reversed_max_speed) state->speed -= reversed_acceleration;
...
GLvoid moto::input(GLint key, GLint action) {
	if (action == GLFW_PRESS) {
		switch (key) {
		case GLFW_KEY_W: state->w_down = true; break;
			case GLFW_KEY_S: state->s_down = true; break;
			case GLFW_KEY_A: state->a_down = true; break;
			case GLFW_KEY_D: state->d_down = true; break;
		}
	}
	if (action == GLFW_RELEASE) {
		switch (key) {
			case GLFW_KEY_W: state->w_down = false; break;
			case GLFW_KEY_S: state->s_down = false; break;
			case GLFW_KEY_A: state->a_down = false; break;
			case GLFW_KEY_D: state->d_down = false; break;
		}
	}
}
